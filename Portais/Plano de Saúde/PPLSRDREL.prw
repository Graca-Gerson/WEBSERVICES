...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³PPRELBEN  ºAutor  ³Totvs               º Data ³  20/02/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Processa Relacao de Movimentacao							  º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function PPRELBEN()
LOCAL cMsgErro	:= ""
LOCAL cArq		:= "PLSRXXX" + CriaTrab(NIL,.F.) + ".HTM"
LOCAL cArqHTM 	:= __RELIMP + cArq
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
//³ Se o diretorio nao existir cria
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
...
If setDirRel()
	...
	if  empty(cTp) .or.  cTp == "5"
		If PLSALIASEXI("B0D") .and. !empty(cNumGuia)
			 B0D->(DbSetOrder(1))
		     If B0D->(MsSeek(xFilial("B0D")+cNumGuia))
				...
				If Len(aGuiaGen) > 0
					...
					Else
						cArqName := PLSTISS5(aGuiaGen, .F.,2,nil, .T., __RELIMP)
					Endif
				Endif
		     	lEntrou := .T.
		     Endif
	    ...
	    If BEA->( MsSeek(xFilial("BEA") + cNumGuia) )
			...
			If Len(aGuiaGen[1]) > 0 
				...
				If cTipoGui == "1" // CONSULTA
					...
					Else
						cArqName := PLSTISS1(aGuiaGen, .F.,,, .T., __RELIMP)
					EndIf
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
user function PLSQUIT()
LOCAL cMsgErro	:= ""
LOCAL aPar 	  	:= paramixb[1]
Local aDados		:= { aPar:Tipo, aPar:CodEmp, aPar:NumCon, aPar:SubCon, aPar:Matric, aPar:Ano }
...
If setDirRel()
	...
	If 	Val(aPar:Ano)>= year(Msdate()) // não podemos imprimir do ano corrente ou superior
		cArqName	:= ""  
		cMsgErro := 'Somente poderá ser impresso anos anteriores !'
	Else
		If findFunction("U_PLSQTDEB")
			//Executa relatório
			A772Acao(aDados, .T., __RELIMP)
		Else
