...
If cTipRel == "4"
	...
	*/
	...
	cSql += " FROM " + RetSqlName(cAlias) + cJoin
	cSql += " INNER JOIN " + RetSqlName('BD7') + " BD7 ON "
	cSql += " BD7_FILIAL = '" + xFilial("BD7") + "' AND BD7_CODOPE = BCI_CODOPE AND BD7_CODLDP = BCI_CODLDP AND BD7_CODPEG = BCI_CODPEG AND BD7_CODRDA = BCI_CODRDA  "
	cSql += " AND BD7_BLOPAG <> '1' AND BD7.D_E_L_E_T_ = ' '  "
...
If AllTrim(TCGetDB()) $ "ORACLE/DB2"
	cSql := "SELECT E1_NOMCLI,E1_EMISSAO,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_MESBASE,E1_ANOBASE,E1_VENCREA,E1_VALOR, TO_CHAR(R_E_C_N_O_) RECNO"
Else
	cSql := "SELECT E1_NOMCLI,E1_EMISSAO,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_MESBASE,E1_ANOBASE,E1_VENCREA,E1_VALOR, CONVERT ( VARCHAR,R_E_C_N_O_ ) RECNO"
EndIf
...
if !lResInt
	cSql3		+= " BD5_CODLDP, BD5_DATPRO, BD5_SENHA, BD5_OPEMOV||'.'||BD5_ANOAUT||'.'||BD5_MESAUT||'.'||BD5_NUMAUT BD5_NUMAUT, BD5_NOMUSR, " + RetSQLName(cAlias) + ".R_E_C_N_O_ RECNO, BD5_SITUAC, BD5_CODPEG "
	cCampos	:= " Origem Guia=BD5_CODLDP, ?=BD5_DATPRO, ?=BD5_SENHA, ?=BD5_NUMAUT, ?=BD5_NOMUSR, ?=BD5_SITUAC"
else
	cSql3 		+= " BE4_CODLDP, BE4_DATPRO, BE4_SENHA, BE4_CODOPE||'.'||BE4_ANOINT||'.'||BE4_MESINT||'.'||BE4_NUMINT BE4_NUMAUT, BE4_NOMUSR, " + RetSQLName(cAlias) + ".R_E_C_N_O_ RECNO, BE4_SITUAC, BE4_CODPEG "	
...
LOCAL nI			:=1
LOCAL lTodas		:= .F. // Lista todas as receitas mesmo as vencidas e com quantidade insufieciente
LOCAL nTamCodRec	:=tamSX3("B1N_CODREC")[1] 
...
cAuxSql1 := "SELECT BBA_STATUS,BBA_CODSEQ,BBA_NROPRO,"
...
cSql   := " SELECT BA1_CODINT XPTO, BA1_CODEMP, BA1_MATRIC, BA1_TIPREG, BA1_DIGITO, BA1_NOMUSR, BA1_CPFUSR, BA1_DATNAS, "
...
Local cAlias		:= ""
Local cAliasAux		:= ""
Local cCampos		:= "CPF=CPF,Nome=NOME,Tipo=TIPO,Grau Parentesco=GRAU,Opera็ใo=OPER"
Local cSql			:= ""
Local cWhere		:= ""
Local cFieldJ		:= ""
Local cAuxSql1   	:= ""
...
If cTipo $ ("1|3") //inclusใo/exclusใo (bloqueado)
	...
	cAlias := "B2N"
	cAuxSql1 := "SELECT B2N_CPFUSR CPF, B2N_NOMUSR NOME, CASE B2N_TIPUSU WHEN '" + cCodTit + "' THEN 'Titular' WHEN '" + cCodDep + "' THEN 'Dependente'  END AS TIPO , BRP_DESCRI GRAU, CASE BBA_TIPMAN WHEN '1' THEN 'Inclusใo' WHEN '2' THEN 'Altera็ใo' WHEN '3' THEN 'Exclusใo' END AS OPER "
...
Else //altera็ใo
	...
	If TRBB7L->(Eof())
		...
		cAlias := "BA1"
		cAuxSql1 := "SELECT BA1_CPFUSR CPF, BA1_NOMUSR NOME, CASE BA1_TIPUSU WHEN 'T' THEN 'Titular' WHEN 'D' THEN 'Dependente'  END AS TIPO , BRP_DESCRI GRAU, CASE BBA_TIPMAN WHEN '1' THEN 'Inclusใo' WHEN '2' THEN 'Altera็ใo' WHEN '3' THEN 'Exclusใo' END AS OPER "
	...
	Else
		...
		cAuxSql1 := "SELECT BA1_CPFUSR CPF, BA1_NOMUSR NOME, CASE BA1_TIPUSU WHEN 'T' THEN 'Titular' WHEN 'D' THEN 'Dependente'  END AS TIPO, "
		cAuxSql1 += " BRP_DESCRI GRAU, CASE BBA_TIPMAN WHEN '1' THEN 'Inclusใo' WHEN '2' THEN 'Altera็ใo' WHEN '3' THEN 'Exclusใo' END AS OPER "
		...
		cWhere += " GROUP BY BA1_CPFUSR, BA1_NOMUSR, BA1_TIPUSU, BRP_DESCRI, BBA_TIPMAN "
	EndIf
...
cSql += " BEA_TIPGUI TIPO,CASE WHEN BEA_LIBERA = '0' THEN 'Execu็ใo' ELSE 'Solicita็ใo' END LIBERA, BEA_NRLBOR LIBORI, BEA_OPEMOV || BEA_CODEMP || BEA_MATRIC || BEA_TIPREG || BEA_DIGITO MATRICULA, BEA_CODRDA CODRDA, BEA_CODLOC CODLOC, 'Nใo' DIGOFF," + RetSqlName(cAlias)+".R_E_C_N_O_ IDENLINHA"
cSql += cSqlB53
cSql += " FROM " + RetSqlName("BEA") 
If (B53->(FieldPos("B53_NMGORI")) > 0)
	cSql += " LEFT JOIN " + RetSqlName("B53")
	cSql += " ON BEA_OPEMOV || BEA_ANOAUT || BEA_MESAUT || BEA_NUMAUT = B53_NUMGUI "
EndIf
...
cSql1 += " BE4_TIPGUI TIPO, 'Solicita็ใo' LIBERA, ' ' LIBORI, BE4_CODOPE || BE4_CODEMP || BE4_MATRIC || BE4_TIPREG || BE4_DIGITO MATRICULA, BE4_CODRDA CODRDA, BE4_CODLOC CODLOC, 'Nใo' DIGOFF, " + RetSqlName(cAlias1)+".R_E_C_N_O_ IDENLINHA"
cSql1 += cSqlB53
cSql1 += " FROM " +  RetSqlName("BE4")
If (B53->(FieldPos("B53_NMGORI")) > 0)
	cSql1 += " LEFT JOIN " + RetSqlName("B53") 
	cSql1 += " ON BE4_CODOPE || BE4_ANOINT || BE4_MESINT || BE4_NUMINT = B53_NUMGUI "
EndIf
...
cSql2 += "  B4A_TIPGUI TIPO, 'Solicita็ใo' LIBERA, B4A_GUIREF LIBORI, B4A_OPEMOV || B4A_CODEMP || B4A_MATRIC || B4A_TIPREG || B4A_DIGITO MATRICULA, '' CODRDA, '' CODLOC, 'Nใo' DIGOFF, " +RetSqlName(cAlias2)+".R_E_C_N_O_ IDENLINHA"
cSql2 += cSqlB53
cSql2 += " FROM " +  RetSqlName("B4A")
If (B53->(FieldPos("B53_NMGORI")) > 0)
	cSql2 += " LEFT JOIN " + RetSqlName("B53") 
	cSql2 += " ON B4A_OPEMOV || B4A_ANOAUT || B4A_MESAUT || B4A_NUMAUT = B53_NUMGUI "
EndIf
...
if lAliB4Q
	...
	cSql3 += "  '11' TIPO, 'Solicita็ใo' LIBERA, B4Q_GUIREF LIBORI,  B4Q_OPEMOV || B4Q_CODEMP || B4Q_MATRIC || B4Q_TIPREG || B4Q_DIGITO MATRICULA, '' CODRDA, '' CODLOC, 'Nใo' DIGOFF, " +RetSqlName(cAlias3)+".R_E_C_N_O_ IDENLINHA"
	cSql3 += cSqlB53
	cSql3 += " FROM " +  RetSqlName("B4Q")
	If (B53->(FieldPos("B53_NMGORI")) > 0)
		cSql3 += " LEFT JOIN " + RetSqlName("B53") 
		cSql3 += " ON B4Q_OPEMOV || B4Q_ANOAUT || B4Q_MESAUT || B4Q_NUMAUT = B53_NUMGUI "
	EndIf
endIf
...
cSql4 += " BD5_TIPGUI TIPO, 'Execu็ใo' LIBERA, BD5_NRLBOR LIBORI, BD5_OPEMOV || BD5_CODEMP || BD5_MATRIC || BD5_TIPREG || BD5_DIGITO MATRICULA, BD5_CODRDA CODRDA, BD5_CODLOC CODLOC, 'Sim' DIGOFF, " + RetSqlName(cAlias4)+".R_E_C_N_O_ IDENLINHA "
if lGoriB53
	cSql4 += ", ' ' NMGORI, ' ' NMGGER "
endIf	
...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ
cWhere  += " WHERE BEA_FILIAL = '" + xFilial(cAlias) + "' AND BEA_TIPO <> '3' "
cWhere  += " AND "+ RetSqlName("BEA")+".D_E_L_E_T_ = ' ' "
cWhere1 += " WHERE BE4_FILIAL = '" + xFilial(cAlias1) + "' "
...
if Len(aWhere) > 1 .And. aWhere[2][2] == "2" //Por beneficiario
	...
	cWhere2 += " AND BEA_OPEMOV || BEA_ANOAUT || BEA_MESAUT || BEA_NUMAUT = B4A_GUIREF "
...
endIf
cWhere4  += " WHERE BD5_FILIAL = '" + xFilial(cAlias4) + "' AND BD5_ORIMOV = '5' "
cWhere4  += " AND "+ RetSqlName("BD5")+".D_E_L_E_T_ = ' ' "
...
If Len(aWhere) > 1
	...
	Else//busca por beneficiแrio
		...
		cWhere +=  " AND BEA_OPEMOV || BEA_CODRDA    IN (" + aWhere[3,2] + ") AND ( (BEA_OPEMOV = '" + asepara[1] + "' AND BEA_CODEMP = '" + asepara[2] + "' AND BEA_MATRIC = '" + asepara[3] + "' AND BEA_TIPREG = '" + asepara[4] + "' AND BEA_DIGITO = '" + asepara[5] + "') OR BEA_MATANT = '" + cNumGuiaMat + "' )"
		cWhere1 += " AND BE4_CODOPE || BE4_CODRDA    IN (" + aWhere[3,2] + ") AND ( (BE4_CODOPE = '" + asepara[1] + "' AND BE4_CODEMP = '" + asepara[2] + "' AND BE4_MATRIC = '" + asepara[3] + "' AND BE4_TIPREG = '" + asepara[4] + "' AND BE4_DIGITO = '" + asepara[5] + "') OR BE4_MATANT = '" + cNumGuiaMat + "' )"
		cWhere2 += " AND ( (B4A_OPEMOV = '" + asepara[1] + "' AND B4A_CODEMP = '" + asepara[2] + "' AND B4A_MATRIC = '" + asepara[3] + "' AND B4A_TIPREG = '" + asepara[4] + "' AND B4A_DIGITO = '" + asepara[5] + "') OR B4A_MATANT = '" + cNumGuiaMat + "' )"
	
		if lAliB4Q
			cWhere3 += " AND ( (B4Q_OPEMOV = '" + asepara[1] + "' AND B4Q_CODEMP = '" + asepara[2] + "' AND B4Q_MATRIC = '" + asepara[3] + "' AND B4Q_TIPREG = '" + asepara[4] + "' AND B4Q_DIGITO = '" + asepara[5] + "') OR B4Q_MATANT = '" + cNumGuiaMat + "' )"
			cWhere3 += " AND B4Q_OPEMOV || B4Q_CODRDA    IN (" + aWhere[3,2] + ") "
		endIf
...
cSql := cSql + cWhere 
cSql += " GROUP BY BEA_OPEMOV || BEA_ANOAUT || BEA_MESAUT || BEA_NUMAUT, BEA_NOMUSR, BEA_DATPRO, BEA_STATUS, BEA_CANCEL, BEA_TIPGUI, BEA_LIBERA, BEA_NRLBOR, BEA_OPEMOV || BEA_CODEMP || BEA_MATRIC || BEA_TIPREG || BEA_DIGITO, BEA_CODRDA, BEA_CODLOC, BEA_STTISS," + RetSqlName(cAlias)+".R_E_C_N_O_ " + cGrpB53

cSql += " UNION " + cSql1 + cWhere1 
cSql += " GROUP BY BE4_CODOPE || BE4_ANOINT || BE4_MESINT || BE4_NUMINT, BE4_NOMUSR, BE4_PRVINT, BE4_STATUS, BE4_CANCEL, BE4_TIPGUI, BE4_CODOPE || BE4_CODEMP || BE4_MATRIC || BE4_TIPREG || BE4_DIGITO, BE4_CODRDA, BE4_CODLOC, BE4_STTISS," + RetSqlName(cAlias1)+".R_E_C_N_O_ " + cGrpB53

cSql += " UNION " + cSql2 + cWhere2
cSql += " GROUP BY B4A_OPEMOV || B4A_ANOAUT || B4A_MESAUT || B4A_NUMAUT, B4A_NOMUSR, B4A_DATSOL, B4A_STATUS, B4A_CANCEL, B4A_TIPGUI, B4A_GUIREF, B4A_OPEMOV || B4A_CODEMP || B4A_MATRIC || B4A_TIPREG || B4A_DIGITO, B4A_STTISS," + RetSqlName(cAlias2)+".R_E_C_N_O_ "  + cGrpB53

if lAliB4Q
	cSql += " UNION " + cSql3 + cWhere3 
	
	If lFPosSTIS 
		cSql += " GROUP BY B4Q_OPEMOV || B4Q_ANOAUT || B4Q_MESAUT || B4Q_NUMAUT, B4Q_NOMUSR, B4Q_DATPRO, B4Q_STATUS, B4Q_CANCEL, B4Q_GUIREF, B4Q_OPEMOV || B4Q_CODEMP || B4Q_MATRIC || B4Q_TIPREG || B4Q_DIGITO, B4Q_STTISS," + RetSqlName(cAlias3)+".R_E_C_N_O_ " + cGrpB53
	Else
		cSql += " GROUP BY B4Q_OPEMOV || B4Q_ANOAUT || B4Q_MESAUT || B4Q_NUMAUT, B4Q_NOMUSR, B4Q_DATPRO, B4Q_STATUS, B4Q_CANCEL, B4Q_GUIREF, B4Q_OPEMOV || B4Q_CODEMP || B4Q_MATRIC || B4Q_TIPREG || B4Q_DIGITO," + RetSqlName(cAlias3)+".R_E_C_N_O_ " + cGrpB53
	EndIf 
endIf

cSql += " UNION " + cSql4 + cWhere4
cSql += " GROUP BY BD5_OPEMOV || BD5_ANOAUT || BD5_MESAUT || BD5_NUMAUT, BD5_NOMUSR, BD5_DATPRO, BD5_SITUAC, BD5_FASE, BD5_TIPGUI, BD5_LIBERA, BD5_NRLBOR, BD5_OPEMOV || BD5_CODEMP || BD5_MATRIC || BD5_TIPREG || BD5_DIGITO, BD5_CODRDA, BD5_CODLOC, " + RetSqlName(cAlias4)+".R_E_C_N_O_ "
...
cCampos  += "0#Justif. Operadora Prot/Guia= " + cListCri + ",1#Itens= " + cItens + ", Origem Recurso=ORIGEM,0#Enviar Anexo=CHAVE"
...
if "ORACLE" $ upper(TCGetDb()) //Nใo remover por causa do STR
